...
| **M3** — J5..J8 PCB sockets have no schematic counterpart | **DEFERRED**. PCB pads on J5..J8 stay on net 0 until schematic-side rework adds J5+J6 (ESP32 socket pair) and J7+J8 (NFC socket pair) with proper pin numbering matching the U3 / U4 wire endpoints, and removes the U3 / U4 placeholder symbols. Non-trivial — requires re-wiring every ESP32 GPIO and every NFC mikroBUS pin one row at a time. |  |
...
| **Pre-existing root-sheet bridge** (spotted during C1 verification — not in original review) | OPEN. Two root-sheet wires connecting Sensors right-edge hier pins to MCU right-edge hier pins cross over IO sub-sheet's left-edge pins at identical Y coordinates, bridging UART_TX/RX with USB_DP/EN. Visible in netlist: `J4 pin 2 → /IO/EN`, `J4 pin 3 → /IO/USB_DP`. Fix during M3. |  |

**Updated routing-readiness verdict: READY** for the first copper routing chunk on the resolved-nets (those that have both schematic and PCB endpoints populated). The user can begin routing now; the J5..J8 ESP32 / NFC socket nets remain unrouted until M3 lands.
