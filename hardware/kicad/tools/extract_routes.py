...
serialize them as a Python data file (`oas_routes.py`) for generate.py to
...
alongside the source-of-truth `generate.py`.
...
# A4 page-centre offset used by generate.py to convert between page and
# PCB-local coordinates. Kept here to convert PCB coordinates as written
# in oas.kicad_pcb (page-space) into the PCB-local frame that generate.py
...
def main() -> None:
    ...
    # post-sort order. This means generate.py's UUIDs are tied to the
    ...
    lines.append("are loaded by generate.py's `_route_autoroute_tracks()` chunk.")
    lines.append("")
