...
- **Board-id block + repo QR code** live in the west pocket (freed by the NFC removal): a 33×33 QR (v4, EC M) linking to the public repo, centred on the central hole's horizontal axis (Y=0), with the board name + version lines centred below. Matrix VENDORED in `boardgen/_qr_data.py` (provenance + regen note there — boardgen stays dependency-free and deterministic); placement in `QR_SILK_*` (`boardgen/_project.py`). Polarity is NORMAL (white silk field = light modules + 4-module quiet zone; dark modules = bare mask) so every scanner reads it. Emitted as run-length-merged filled `gr_poly` rects — the stage-13 silk stroke lifter EXEMPTS filled polys (a lifted 0-width stroke would grow each rect 0.1 mm/side and shrink dark modules 0.5→0.3 mm, breaking scannability). End-to-end verified: the QR decodes from `renders/pcb/2d-top.png`.
