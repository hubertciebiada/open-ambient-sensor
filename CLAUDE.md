...
**v0.53 placement in progress (signal routing deferred to GitHub issue #8).** The board layout is being reworked one placement task at a time (issue #2 SEN66 recess cutout landed first). To avoid re-routing after every task, signal routing is DELIBERATELY absent for now: `ROUTING_CHUNKS = ("gnd",)` emits GND pours only, so DRC is 0 violations but non-GND pads read as unconnected (`pipeline/generic/03_drc.py` `EXPECTED_UNCONNECTED` holds the exact pours-only baseline, temporary until the one-shot re-route). `oas_routes.py` is retained as a reference snapshot but not emitted; route-dependent checks/tests (stage 26 I2C rise-time, the extract-routes round-trip, the snapshot-sanity suites) self-skip while `"autoroute"` is off. The full Freerouting re-route happens ONCE at the end under issue #8, which must return `EXPECTED_UNCONNECTED` to 0.
...
- **ESP32-C6 DevKitM-1 placement** (v0.53, GitHub issue #3): the DevKit shadow + its J5/J6 pin sockets sit 7.5 mm WEST of the pre-v0.53 position (`ESP32_ANCHOR_X = −35.26`), seating the module next to the THT bulk caps — the body east edge (+13.00) clears the C3 Ø8 mm can's west rim (+21.75) by 8.75 mm. The DevKitM-1 is NOT a plain rectangle: the ESP32-C6-MINI-1 PCB antenna adds a 13.20 × 5.37 mm tab overhanging the west short edge (both dimensions read from the Espressif dimensions PDF). The mech-ref draws the FULL true outline (body + tab) on **F.Fab only** — after the move its F.SilkS body outline would fall off the board at the NW corner AND cross the buck / U1 copper, so no F.SilkS is emitted (the J5/J6 socket silk frames + the board-level "ESP32-C6 DevKitM-1" F.Fab label document the module). **KNOWN, user-accepted overhang** (pending an enclosure-wall check): the body NW corner is 1.26 mm past the R60 outline; the antenna tab far corner stays 0.11 mm INSIDE the outline (the tab itself does not overhang). The mech-ref carries NO F.CrtYd (daughterboards sit ~8.6 mm up on their sockets, so SMDs live under them — U1 TO-263 4.83 mm < the 5.5 mm socket budget); the Z-clearance guardrail's MOD1 shadow is extended west to cover the tab so U1-under-tab is actually checked (and passes).
