...
- **ESP32-C6 DevKitM-1 placement** (v0.53, GitHub issue #3 + its review): the J5/J6 pin sockets sit exactly 7.5 mm WEST of the v0.51 position (pin 1 at PCB X = −29.89, user-measured on the physical board — the sockets are the PHYSICAL datum), seating the module next to the THT bulk caps — the body east edge (+16.795) clears the C3 Ø8 mm can's west rim (+21.75) by 4.955 mm, and the SEN66 cutout west edge (+20.50) by 3.7 mm. The DevKitM-1 is NOT a plain rectangle: the ESP32-C6-MINI-1 PCB antenna adds a 13.20 × 5.37 mm tab overhanging the west short edge (both dimensions read from the Espressif dimensions PDF; see Lesson 22 for the pin-1 offset the same review corrected). `ESP32_ANCHOR_X = −31.465` is DERIVED (pin-1 datum − 1.575 pin offset); body X −31.465..+16.795. **No board-edge overhang**: the body NW corner sits 0.847 mm INSIDE the R60 outline and the antenna tab far corner 2.62 mm inside (the earlier "1.26 mm accepted overhang" was an artifact of the wrong pin offset — Lesson 22). The mech-ref draws the FULL true outline (body + tab) on F.Fab and four corner L-ticks on F.SilkS (a full silk rect is impossible there — the short edges would cross the U1 lead-pad ends, the J5/J6 socket-frame ends and the C2 pad column; the ticks mark the corners and skip the congested mid-spans). It carries NO F.CrtYd (daughterboards sit ~8.6 mm up on their sockets, so SMDs live under them — U1 TO-263 4.83 mm < the 5.5 mm socket budget); the Z-clearance guardrail's MOD1 shadow is extended west over the tab so U1 is actually checked (and passes).
