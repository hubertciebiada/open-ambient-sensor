...
The ring ran at 25-30 % on the light's scale — LED output 5-9 of 255. Two complaints came from that one fact. (a) The 0-255 slider was "too big": light brightness 1..40 of 255 (the bottom 16 %) all land on the same dimmest step — output 1 on ESPHome 2026.9+, whose gamma LUT never returns 0 for a non-zero input; on 2026.8 part of that band rounds to 0, i.e. dark. "Ring Max Brightness" plus a hidden 5 % night multiplier on top of that scale was opaque — night mode resolved to that dimmest step (or off), which read as "the ring turns itself off". (b) "AQI Breathing" (colour × cosine envelope, left to the gamma table) had only ~5 distinct outputs between trough and peak — visible jumps — and its 15 % pre-gamma floor sat on the dimmest step for ~40 % of each cycle, so every swell seemed to start late. Fix: the light entity went **internal** and the user-facing controls became plain entities in the order the owner specified (on/off, mode, brightness, night mode, night brightness ≤ brightness, night hours); brightness is **ten levels matched to distinct LED outputs over the full range** (1, 2, 3, 5, 10, 19, 36, 69, 133, 255 — geometric, ~1.9x per step, level 4 anchored at the old 25 % = the temperature-calibration setting), each level's light brightness = the top of its output band computed against ESPHome's own integer pipeline (`esp_scale8_twice` + the uint16 gamma LUT; identical on 2026.8 and 2026.9); the breathing effect computes final LED values itself: peak read back through the light's pipeline (`get_*_raw()`), a perceptually even envelope with a floor at 20 % of peak output (never below 1 step), **spatial dithering across the 7 LEDs** (1/7-step resolution behind the diffuser), written through an identity `ESPColorCorrection`. Rules: never offer a raw 0-255 control for a dim indicator; simulate the exact integer pipeline before choosing numbers (and check it across ESPHome versions — the rounding changed between 2026.8 and 2026.9); and when a smooth animation must run at a handful of output steps, dither across LEDs rather than trusting the gamma table.

**Follow-up — dither brightness, never hue.** The first dithered breath dithered each colour channel on its own. A yellow-green's weak red channel — ~0.05 of a step at brightness 4, pushed up to a whole step by 2026.9's never-zero gamma LUT — stayed lit on some LEDs while green dimmed, so single LEDs went from the peak's 20 % red share up to 100 % on every exhale: the ring blinked red (owner report, reproduced in the host sim). Fix: the weaker channels follow the strongest one on each LED as a fixed fraction of it (the colour's mix after the gamma), and since a mix cannot keep its hue at 1-3 steps, a breath whose trough is under 7 steps (levels 1-6) rounds the mix to whole fractions — green / yellow / red; the half-output threshold is also the CIE u'v' hue midpoint for the SK6812SIDE-A's datasheet intensities (R 345 / G 930 mcd mid-bin). Rule: at a handful of output steps a mixed colour cannot dim without changing hue — spend the dithering on brightness only, and give the breath a colour its dimmest step can show.

### 28. A power-on reset says the chip lost power, not for how long — measure the downtime before choosing a cold start

The SEN66 STAR-Engine offsets are volatile (zero after the module reset at every boot) and were re-sent in one of two ways: after a cold start they ramp in with the enclosure's warm-up time constant (τ = 1800 s), after a warm start they apply at once. The firmware told the two apart by the ESP32 reset reason alone: `power-on event` = cold. A few-second 24 V interruption on units that had run for hours is a power-on reset too — on a warm enclosure. The units took the cold path, restarted the −3.4 K offset from zero and ramped it with τ: the reading peaked +2.9 K above its value before the interruption and was still ~1.4 K high 26 min later, long enough for climate automations to act on it (owner incident, 2026-09-25). Fix (`packages/air-quality.yaml`, owner spec): the firmware stamps the network time into NVS once a minute and flushes it at once (left to `flash_write_interval` the stamp lags up to 2 min, and lag reads as downtime). After a power-on it waits for the clock (≤ 60 s of uptime) and takes the downtime d from that stamp to the moment the boot began. The enclosure is modelled as cooling with the τ it warms with, so it still holds f = e^(−d/τ) of each offset: that part goes into a spare slot (2 / 3) at tau 0, the rest into the slot's own ramp. Slots add up, so the total follows O·(1 − (1 − f)·e^(−t/τ)) using only behaviour the cold and warm paths already relied on; rewriting one slot twice (O·f at tau 0, then O at τ) would need the ramp to start from the slot's current value, which is unverified on the module. Any other reset keeps f = 1; no clock or no stamp keeps the old rule (power-on = cold). Rules: a reset reason describes the chip, not its surroundings — when behaviour depends on time elapsed across a power loss and there is no RTC battery, persist a heartbeat and measure; and prefer composing verified module behaviour over a design that needs a new assumption confirmed first. ⚠ Open: the same-τ cooling assumption is what the 30-minute-outage bench test checks (f0 ≈ 0.37, reading within ±0.3 K of a reference over the first 30 min).
...
- STAR offsets after a power-on: the downtime (network-time stamp in NVS once a minute, compared at boot once the clock syncs, ≤ 60 s) decides how much applies at once — f = e^(−d/τ) into spare slots 2 / 3 at tau 0, the rest ramps in slots 0 / 1. Non-power-on resets apply everything at once; no clock / no stamp = the old cold start. `STAR-Engine State` shows `downtime …, f0 …, f1 …`. See Lesson 28.
