...
- LED ring (`packages/leds.yaml`): the light entity is **internal**; the dashboard card shows, in this order, Ring (switch), Ring Mode (select: AQI Breathing / AQI Solid / Dot Chase), Ring Brightness (1-10 = the LEDs' full range in ~1.85x steps), Ring Night Mode (switch), Ring Night Brightness (0..Ring Brightness, snapped back if set higher), Ring Night Start / End (`datetime` time entities, 22:00 → 07:00), then a Ring Status line. A 1 s schedule tick is the light's only writer: it derives on/off, effect and brightness from those settings, the clock (last known day/night kept across reboots) and the critical-air flag (solid red, at least brightness 7 by day, the night brightness at night). See Lesson 27.
