...
**v0.50 routing rework (in progress).** Placement fully reworked and committed (DRC 0, build 30/30): USB-C case-wall cutout made non-blocking for the autorouter; input-protection cluster re-columned into two vertical stacks under ZT1; the 20-part buck section under the ESP32 re-spread from 3 cramped bands into a clean 2-row grid; C12 NFC decoupling moved to the J7 +3V3 pin; 5 LED-ring decoupling caps pulled toward the centre hole. Freerouting (Docker, diagnostic) reaches **88/89** on the reworked placement — the old layout had 2 *structurally* unroutable nets (`/IO/BTN`, `Earth_Protective`, central corridor); the rework fixed both. The last net `Net-(D14-DOUT)` is corridor-congestion-limited (proven: 4 distinct LED-placement configs all gave bit-identical autoroute) and was hand-routed in KiCad → **89/89 full coverage**.

The 89/89 routing snapshot is captured in `oas_routes.py` (405 segments + 22 vias, via `tools/extract_routes.py`) but is **NOT yet active**: `ROUTING_CHUNKS` is still `("gnd",)`, so `build.py` produces the GND-only board (30/30 PASS) and the committed `oas.kicad_pcb` is GND-only. Refinement work to activate it: (1) fix the `/IO/BTN` B.Cu track — it sits 0.215 mm from the central-hole edge, needs 0.30 mm (a Freerouting artifact — nudge the offending segment in `oas_routes.py`); (2) refill the GND zone after replay; (3) add the autoroute chunk to `ROUTING_CHUNKS` and rebuild to 30/30. NOTE: the working-tree `oas.kicad_pcb` may carry the hand-assembled routed board (Freerouting SES import + the D14-DOUT trace) — `build.py` regenerates it GND-only; the routing is safe in `oas_routes.py`.
...
### Hardware
- [ ] **Routing rework (v0.50) — activate the captured routing.** Placement done + committed; the 89/89 routing is captured in `oas_routes.py` (405 seg + 22 via) but not active. To finish: fix the `/IO/BTN` central-hole edge-clearance violation (0.215 mm < 0.30 mm), refill the GND zone, flip `ROUTING_CHUNKS` to enable the autoroute chunk, rebuild to 30/30, commit.
