...
| Visual indicator | **11 × SK6812-SIDE** (4020 side-emit) on Ø22 mm pitch ring around the central cable hole, driven by GPIO 8 from the LM2596S +5V rail | 1-wire RMT, WS281x protocol | confirmed v0.17 (D20 slot vacated for the J1 24 V terminal block; v0.16 had 12 LEDs, see OPEN ISSUE #2 resolution) |
...
- **v0.16** — AQI status LED ring added (resolves OPEN ISSUE #2). *Originally 12 × SK6812-SIDE; v0.17 dropped 1 LED (D20) to open a corridor for the J1 24 V terminal block, so the final ring has 11 LEDs.*
