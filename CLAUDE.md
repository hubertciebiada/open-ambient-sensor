...
- [ ] **JLCDFM re-run on the post-issue-#9 gerbers — NOW DUE.** The full re-route is done (375 seg + 38 via), so the copper, drill file and silk are all final for this revision. Baseline to compare against: 0 Danger + the 4 proven mask-expansion phantoms above. Watch specifically for the new GND rescue vias sitting ~0.6–1.5 mm off their pads (Lesson 25) and for the solid SMD-to-pour connections introduced by `connect_pads thru_hole_only` (Lesson 24). Run it before any order.
